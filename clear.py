...
if st.button("▶️ Run Simulation"):
    ...
    # ax2.plot(actual_df["Simulation Week"], actual_df["Ships Completed (Actual)"], marker='o', label="Actual")
    # ax2.plot(ideal_df["Simulation Week"], ideal_df["Ships Completed (Ideal)"], marker='x', linestyle='--', label="Ideal")
    # ax2.set_xlabel("Simulation Week")
    # ax2.set_ylabel("Cumulative Ships Completed")
    # ax2.set_title("S-Curve: Ship Completion vs Simulation Time")
    # ax2.legend()
    # plt.tight_layout()
    # st.pyplot(fig2)
    
    # ---- S-Curve with Partial Completion ----
    st.subheader("📊 S-Curve: Actual vs Ideal Ship Completion (with Partial Progress)")
    ...
    for week in timeline:
        ...
        for _, row in df_records.iterrows():
            ...
            # Partial completion fraction for this week
            if week < stage_start:
                frac = 0
            elif week >= stage_end:
                frac = 1
        ...
        # Normalize by total number of stages across all ships
    ...
    # Ideal line (linear growth to 100%)
    ideal_df = pd.DataFrame({"Week":timeline,"Completion (%)": [(week/sim_time)*100 for week in timeline]})

    # Plot
    fig3, ax3 = plt.subplots(figsize=(10,5))
    ax3.plot(s_curve_df["Week"], s_curve_df["Completion (%)"], label="Actual (with Partial Progress)", color="blue")
    ax3.plot(ideal_df["Week"], ideal_df["Completion (%)"], label="Ideal Linear", color="orange", linestyle="--")
